...
async function inserirDocs(supabase, linhas) {
  ...
  for (const bloco of pedacos(linhas, LOTE_INSERT)) {
    ...
    for (const linha of bloco) {
      ...
      if (!e1 && um) idPorHash.set(um.content_hash, um.id);
      // Erro por linha e quase sempre duplicata numa corrida: ignorar e seguir e o
      // comportamento certo — o ato ja esta no acervo.
    }
  }
  return idPorHash;
}
...
async function fontesDoDia(supabase, datas) {
  ...
  for (const bloco of pedacos([...new Set(datas)], LOTE_DEDUPE)) {
    const { data, error } = await supabase
      .from("documents")
      .select("published_at, metadata")
      .eq("source_name", "DOU")
      .in("published_at", bloco);
    if (error) throw error;
    for (const d of data || []) {
      if (!porData.has(d.published_at)) porData.set(d.published_at, new Set());
      porData.get(d.published_at).add(d.metadata?.fonte || "inlabs");
    }
  }
  return porData;
}
...
async function persistDou(supabase, records, opcoes = {}) {
  ...
  let idPorHash;
  try {
    idPorHash = await inserirDocs(supabase, linhas);
  } catch (e) {
    if (!/themes/i.test(String(e.message || ""))) throw e;
    // Coluna themes ausente (migracao M14 nao aplicada): reinsere sem ela.
    idPorHash = await inserirDocs(supabase, linhas.map(({ themes, ...resto }) => resto));
  }
