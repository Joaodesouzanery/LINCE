...
function computeVotacaoFidelidade(delibs, agenciaId) {
    ...
    for (const v of d.votos ?? []) {
      ...
      if (!map.has(v.diretor_id)) {
        map.set(v.diretor_id, {
          diretor_id: v.diretor_id,
          diretor_nome: v.diretor_nome ?? v.diretor_id,
          total_votos: 0, votos_nominais: 0, votos_orientados: 0, votos_divergentes: 0,
        });
      }
      const s = map.get(v.diretor_id);
      s.total_votos++;
      if (v.is_nominal) s.votos_nominais++;
      // Denominador honesto da fidelidade: votos com orientacao resolvivel. Para
      // deliberacoes de agencia (sem o campo orientavel) `!== false` conta TODOS —
      // comportamento identico ao anterior; so o legislativo passa a excluir os sem orientacao.
      if (v.orientavel !== false) s.votos_orientados++;
  ...
  return [...map.values()].map((s) => ({
    ...s,
    // Fidelidade sobre os votos ORIENTAVEIS; null quando nenhum tem orientacao conhecida.
    taxa_fidelidade: s.votos_orientados > 0
      ? ((1 - s.votos_divergentes / s.votos_orientados) * 100).toFixed(1)
      : null,
    cobertura_orientacao: s.total_votos > 0
      ? ((s.votos_orientados / s.total_votos) * 100).toFixed(0)
      : "0",
  }));
}
