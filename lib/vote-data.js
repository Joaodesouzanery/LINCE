...
async function loadVotacoesLeg(supabase) {
  ...
  for (const vt of votesRaw) {
    ...
    // orientavel = orientacao do PROPRIO partido resolvida (nao-nula e nao "Liberado").
    // So esses votos entram no DENOMINADOR da fidelidade — nao inflar contando os
    // sem orientacao (rotulo generico "Maioria"/"Governo" -> NULL) como "fiel".
    const orientavel = !!(vt.orientacao && !/liber/i.test(vt.orientacao));
    votesByVot.get(vt.votacao_id).push({
      ...
      is_divergente: !!(orientavel && orientMapped && votoMapped !== orientMapped),
      is_nominal: true,
    });
  }
