...
async function downloadSection(cookie, date, section) {
  ...
  // ZIP magic bytes: PK (0x50 0x4B). Se não começar assim, é HTML de "sem edição".
  if (buffer[0] !== 0x50 || buffer[1] !== 0x4B) {
    if (debug) console.log(`    [DEBUG ${date} S${section}] sem edição nesse dia/seção (resposta não é ZIP)`);
    return [];
  }
  let entries;
  try {
...
module.exports = { collectDou, login, parseArticle, matchAgency, extractPeopleFromAto, AuthError };
