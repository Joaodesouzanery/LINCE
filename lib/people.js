...
async function upsertPerson(supabase, { full_name, cpf, role, agency_id, source_profile_url, external_ids, uf, strictExternal }) {
  ...
  // .limit(1) em TODAS as buscas por nome/CPF: sem ele, um nome que ja existe em
  // DUPLICATA faz o maybeSingle() devolver PGRST116 com data=null; o erro e descartado
  // e o fluxo cai no insert do fim, criando MAIS uma duplicata (e a proxima chamada
  // repete, auto-amplificando). Medido em producao: 18 grupos de normalized_name e 21
  // de normalized_key duplicados — quase todos PARLAMENTARES, que sao justamente os
  // alvos da porta-giratoria. Nenhum desses indices e unique (schema.sql:258-260).
  if (!found && cpfDigits) {
    const { data } = await supabase.from("people").select("*").eq("cpf", cpfDigits).limit(1).maybeSingle();
  ...
  if (!found && !strictExternal && normalized) {
    const { data } = await supabase.from("people").select("*").eq("normalized_name", normalized).limit(1).maybeSingle();
    found = data || null;
  }
  if (!found && !strictExternal && nameKey) {
    const { data } = await supabase.from("people").select("*").eq("normalized_key", nameKey).limit(1).maybeSingle();
    found = data || null;
  }
