...
async function buildPainelDigest(supabase, painelId, { since, byIngestion = false } = {}) {
  ...
  const [propsRes, stakeRes, orgaoRes, votRes, pautaRes] = await Promise.allSettled([
  ...
  const counts = { proposicoes: proposicoes.length, stakeholders: stakeholders.length, orgaos: orgaos.length, novas_votacoes: novas_votacoes.length, na_pauta: na_pauta.length };
  return { painel, since: sinceIso, proposicoes, stakeholders, orgaos, novas_votacoes, na_pauta, counts, has_novidade: novas_votacoes.length > 0 || na_pauta.length > 0 };
}
...
function toText(digest) {
  ...
  const head = `📋 Painel "${p.nome}"${p.cliente ? ` (${p.cliente})` : ""} — ${nv.length} votação(ões) · ${np.length} na pauta (desde ${d10(digest.since)})`;
