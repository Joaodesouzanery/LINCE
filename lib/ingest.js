// Logica compartilhada de pos-processamento de atos do DOU:
// cria diretores (people), mandatos (mandates) e conexoes (relationships)
// a partir dos atos de pessoal (Secao 2), combinando regex + entidades de IA.
// Tambem: matcher de monitores de vigilancia (usado pelo cron E pelo runner local).
const { extractPeopleFromAto } = require("./dou");
const { upsertPerson } = require("./people");
const { normalizeName, normalizeNameKey } = require("./text");

// Processa um ato de pessoal ja gravado em `documents`.
// doc = { id, agency_id, agency_acronym, published_at, title, text, aiEntities }
// Retorna { people, mandates, relationships } (contagens).
async function processPeopleFromDoc(supabase, doc) {
  // 1) Pessoas via regex (sempre disponivel, conf 0.6)
  const regexPeople = extractPeopleFromAto(doc.text).map((p) => ({ ...p, confidence: 0.6 }));
  // 2) Pessoas via IA (quando houver, conf 0.9) - merge por nome
  const aiPeople = (doc.aiEntities || [])
    .filter((e) => e.kind === "person" && e.name)
    .map((e) => ({ name: e.name, role: e.role || "Dirigente", action: "nomeacao", confidence: 0.9 }));

  const byName = new Map();
  for (const p of [...regexPeople, ...aiPeople]) {
    const key = p.name.toUpperCase();
    const prev = byName.get(key);
    if (!prev || p.confidence > prev.confidence) byName.set(key, p);
  }

  let people = 0, mandates = 0, rels = 0;
  const relsToInsert = [];

  for (const p of byName.values()) {
    const person = await upsertPerson(supabase, {
      full_name: p.name,
      role: p.role,
      agency_id: doc.agency_id
    });
    people++;

    // Mandato (idempotente por person+agency+ato)
    const { data: existing } = await supabase
      .from("mandates")
      .select("id")
      .eq("person_id", person.id)
      .eq("agency_id", doc.agency_id)
      .eq("appointment_act", doc.title)
      .maybeSingle();
    if (!existing) {
      await supabase.from("mandates").insert({
        person_id: person.id,
        agency_id: doc.agency_id,
        role: p.role,
        started_at: p.action === "nomeacao" ? doc.published_at : null,
        ended_at: p.action === "exoneracao" ? doc.published_at : null,
        appointment_act: doc.title,
        source_document_id: doc.id,
        confidence_score: p.confidence
      });
      mandates++;
    }

    relsToInsert.push({
      from_kind: "agency",
      from_id: doc.agency_id,
      to_kind: "person",
      to_id: person.id,
      relationship: "employs",
      source_document_id: doc.id,
      confidence_score: p.confidence,
      metadata: { action: p.action, role: p.role }
    });

    // F-INT1 (F3): materializa a MENCAO documento->pessoa (enum 'mentions' existia
    // sem nenhum produtor). Destrava o DOU no grafo e "atos que citam" nos dossies.
    relsToInsert.push({
      from_kind: "document",
      from_id: doc.id,
      to_kind: "person",
      to_id: person.id,
      relationship: "mentions",
      source_document_id: doc.id,
      confidence_score: p.confidence,
      metadata: { action: p.action, role: p.role, agency_id: doc.agency_id }
    });

    // Alertas por regra (sem IA): nomeacao/exoneracao -> alert
    if (p.action === "nomeacao" || p.action === "exoneracao") {
      await supabase.from("alerts").upsert({
        alert_type: p.action,
        severity: "info",
        target_kind: "person",
        target_id: person.id,
        title: `${p.action === "nomeacao" ? "Nomeação" : "Exoneração"}: ${p.name}`,
        body: `${p.role} na ${doc.agency_acronym || "agência"}. Ato: ${(doc.title || "").slice(0, 120)}`,
        source_document_id: doc.id,
        metadata: { agency_id: doc.agency_id, agency_acronym: doc.agency_acronym }
      }, { onConflict: "source_document_id,target_id,alert_type", ignoreDuplicates: true });
    }
  }

  if (relsToInsert.length) {
    // ON CONFLICT DO NOTHING: evita duplicatas se relationships tiver constraint unique
    await supabase.from("relationships").upsert(relsToInsert, {
      onConflict: "from_kind,from_id,to_kind,to_id,relationship",
      ignoreDuplicates: true
    });
    rels = relsToInsert.length;
  }

  // Alertas de sanção por palavra-chave no texto do ato.
  // F-INT1 (F2): FRONTEIRA DE PALAVRA no texto normalizado — a substring crua casava
  // "multar"/"multarei" e qualquer menção passageira, e esse alerta (severity high)
  // dominava o componente de sinais do score da agência.
  const normText = normalizeName(doc.text || "");
  const SANCAO_RE = /(^| )(SANCAO|SANCOES|MULTA|MULTAS|INFRACAO|INFRACOES|AUTUACAO|AUTUACOES|EMBARGO|EMBARGOS|INTERDICAO|INTERDICOES)( |$)/;
  if (SANCAO_RE.test(normText)) {
    await supabase.from("alerts").upsert({
      alert_type: "sancao",
      severity: "high",
      target_kind: "agency",
      target_id: doc.agency_id,
      title: `Ato sancionatório: ${doc.agency_acronym || "agência"}`,
      body: (doc.title || "").slice(0, 200),
      source_document_id: doc.id,
      metadata: { agency_id: doc.agency_id }
    }, { onConflict: "source_document_id,target_id,alert_type", ignoreDuplicates: true });
  }

  return { people, mandates, relationships: rels };
}

// ── Monitores de vigilância ────────────────────────────────────────────────
// Roda a cada ingestao: cada doc novo e testado contra os monitores ativos e
// gera alerts com alert_type='monitor' (dedupe por doc+monitor via onConflict).

async function loadActiveMonitors(supabase) {
  const { data } = await supabase.from("monitors").select("*").eq("active", true);
  return data || [];
}

// Regex tolerante a pontuacao para CPF/CNPJ no texto do ato.
// Nao usar onlyDigits(texto inteiro): concatenar todos os digitos do ato cria
// falsos positivos entre numeros vizinhos.
function docNumberRegex(digits) {
  return new RegExp(digits.split("").join("[.\\/\\- ]?"));
}

// F-INT1: match com FRONTEIRA DE PALAVRA (substring pura casava "ANA" em "ANALISE").
// haystack normalizado usa espacos como separador -> basta cercar de espacos.
function hasTermWithBoundary(haystack, needle) {
  if (!needle) return false;
  return ` ${haystack} `.includes(` ${needle} `);
}
// Keyword: fronteira so no INICIO (preserva monitores salvos como radical: "regulat"
// segue casando "regulatorio"; mas "ANA" nao casa mais "PANAMA").
function hasTermWithPrefixBoundary(haystack, needle) {
  if (!needle) return false;
  return ` ${haystack}`.includes(` ${needle}`);
}
// Nome com ordem invertida: os tokens do nome precisam aparecer ADJACENTES no texto
// (janela contigua de N palavras, qualquer ordem) — "bag of words" no ato inteiro
// casava nomes montados de PESSOAS DIFERENTES em atos de pessoal com varios nomeados.
function hasAdjacentTokens(haystackTokensArr, nameTokens) {
  const n = nameTokens.length;
  if (n < 2 || haystackTokensArr.length < n) return false;
  const target = [...nameTokens].sort().join(" ");
  for (let i = 0; i <= haystackTokensArr.length - n; i++) {
    const win = haystackTokensArr.slice(i, i + n);
    if ([...win].sort().join(" ") === target) return true;
  }
  return false;
}

// Puro/sincrono (testavel): monitores x contexto de um doc -> alertas a emitir.
// docCtx = { docId, title, text, agencyId, agencyAcronym, publishedAt, aiEntities }
function matchMonitorsForDoc(monitors, docCtx) {
  if (!monitors.length) return [];
  const rawJoined = `${docCtx.title || ""} ${(docCtx.text || "").slice(0, 20000)}`;
  const haystack = normalizeName(rawJoined);
  // F-INT1: ordem invertida de nome via janela ADJACENTE de tokens (nao bag-of-words).
  const haystackTokensArr = haystack.split(" ").filter(Boolean);
  const entityKeys = new Set(
    (docCtx.aiEntities || []).map((e) => normalizeName(e?.name || "")).filter(Boolean)
  );
  const entityNameKeys = new Set(
    (docCtx.aiEntities || []).map((e) => normalizeNameKey(e?.name || "")).filter(Boolean)
  );
  const hits = [];
  for (const m of monitors) {
    let matched = null;
    if (m.kind === "agency") {
      matched = m.agency_id && m.agency_id === docCtx.agencyId ? "agency" : null;
    } else if (m.kind === "keyword") {
      matched = hasTermWithPrefixBoundary(haystack, m.normalized_pattern) ? "text" : null;
    } else {
      // person | company
      const nameKey = normalizeNameKey(m.pattern || m.normalized_pattern || "");
      const nameTokens = nameKey.split(" ").filter(Boolean);
      if (m.cpf_cnpj && docNumberRegex(m.cpf_cnpj).test(docCtx.text || "")) matched = "document_number";
      else if (hasTermWithBoundary(haystack, m.normalized_pattern)) matched = "text";
      // ordem invertida: tokens do nome ADJACENTES no texto (janela contigua, qualquer ordem)
      else if (hasAdjacentTokens(haystackTokensArr, nameTokens)) matched = "text_tokens";
      else if (entityKeys.has(m.normalized_pattern) || (nameKey && entityNameKeys.has(nameKey))) matched = "ai_entity";
    }
    if (!matched) continue;
    hits.push({
      monitorId: m.id,
      alert: {
        alert_type: "monitor",
        severity: m.severity || "medium",
        target_kind: "monitor",
        target_id: m.id,
        title: `Monitor: ${m.label}`,
        body: (docCtx.title || "").slice(0, 200),
        source_document_id: docCtx.docId,
        metadata: {
          monitor_id: m.id,
          monitor_kind: m.kind,
          pattern: m.pattern,
          matched,
          // agency_id: permite o type=score bucketear o alerta na agencia do ato
          agency_id: docCtx.agencyId || null,
          agency_acronym: docCtx.agencyAcronym || null,
          published_at: docCtx.publishedAt || null
        }
      }
    });
  }
  return hits;
}

// Atualiza hit_count/last_hit_at dos monitores por INSERTS REAIS.
// monitorHits = Map(monitorId -> n de alertas NOVOS). Read-modify-write best-effort:
// ha mais de um escritor (cron, backfill, loaders) — perda eventual de contagem e aceita.
async function bumpMonitorHits(supabase, monitorHits) {
  const now = new Date().toISOString();
  for (const [monitorId, n] of monitorHits) {
    if (!n) continue;
    const { data: cur } = await supabase.from("monitors").select("hit_count").eq("id", monitorId).maybeSingle();
    await supabase.from("monitors")
      .update({ hit_count: (cur?.hit_count || 0) + n, last_hit_at: now })
      .eq("id", monitorId);
  }
}

// Grava os alertas (dedupe) e atualiza os contadores dos monitores.
// F-INT1: incrementa hit_count SO pelos alertas realmente INSERIDOS (o upsert com
// ignoreDuplicates descarta repetidos — antes o contador inflava a cada re-run/backfill
// sobre os mesmos docs). Retorna o nº de alertas novos.
async function flushMonitorAlerts(supabase, alertRows, _monitorHits) {
  if (!alertRows.length) return 0;
  const { data: insertedRows, error } = await supabase.from("alerts").upsert(alertRows, {
    onConflict: "source_document_id,target_id,alert_type",
    ignoreDuplicates: true
  }).select("id, target_id");
  if (error) return 0;
  const realHits = new Map();
  for (const r of insertedRows || []) realHits.set(r.target_id, (realHits.get(r.target_id) || 0) + 1);
  await bumpMonitorHits(supabase, realHits);
  return (insertedRows || []).length;
}

module.exports = { processPeopleFromDoc, loadActiveMonitors, matchMonitorsForDoc, flushMonitorAlerts, bumpMonitorHits };
