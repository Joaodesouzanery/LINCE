...
// === M22 (Painel F4): agenda/eventos da Camara + pauta ("na pauta") ===

// Eventos da Camara numa janela (dataInicio/dataFim = YYYY-MM-DD). Paginado.
async function fetchEventos({ dataInicio, dataFim } = {}) {
  const out = [];
  for (let pagina = 1; pagina <= 20; pagina++) {
    const params = new URLSearchParams({ ordem: "ASC", ordenarPor: "dataHoraInicio", itens: "100", pagina: String(pagina) });
    if (dataInicio) params.set("dataInicio", dataInicio);
    if (dataFim) params.set("dataFim", dataFim);
    const r = await fetchJson(`${CAMARA_BASE}/eventos?${params.toString()}`);
    if (!r.ok) return { ok: out.length > 0, error: r.error, items: out };
    const dados = r.body?.dados || [];
    for (const e of dados) {
      const org = (e.orgaos || [])[0] || {};
      out.push({
        externalId: e.id != null ? String(e.id) : null,
        dataInicio: e.dataHoraInicio || null,
        dataFim: e.dataHoraFim || null,
        situacao: e.situacao || null,
        tipo: e.descricaoTipo || null,
        descricao: cleanHtmlText(e.descricao || "").slice(0, 600) || null,
        orgaoSigla: org.sigla || null,
        orgaoNome: org.nome || null,
        local: (e.localCamara && e.localCamara.nome) || e.localExterno || null,
        url: e.id != null ? `https://www.camara.leg.br/evento-legislativo/${e.id}` : null
      });
    }
    if (dados.length < 100) break;
  }
...
// Pauta de um evento — SO itens que referenciam uma proposicao (o elo com o painel).
async function fetchPautaEvento(eventoId) {
  const r = await fetchJson(`${CAMARA_BASE}/eventos/${encodeURIComponent(eventoId)}/pauta`);
  if (!r.ok) return { ok: false, error: r.error, items: [] };
  const items = (r.body?.dados || [])
    .filter((it) => it.proposicao_ && it.proposicao_.id != null)
    .map((it) => {
      const p = it.proposicao_;
      return {
        proposicaoId: `camara:${p.id}`,
        ordem: it.ordem != null && !Number.isNaN(Number(it.ordem)) ? Number(it.ordem) : null,
        topico: it.topico || null,
        titulo: it.titulo || (p.siglaTipo ? `${p.siglaTipo} ${p.numero}/${p.ano}` : null),
        situacaoItem: it.situacaoItem || null,
        regime: it.regime || null
      };
    });
  return { ok: true, items };
}
