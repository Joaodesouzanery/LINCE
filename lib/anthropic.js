...
// Extrai os temas itemizados de um ato que APROVA/atualiza a Agenda Regulatoria
// de uma agencia (o texto ja esta em documents.extracted_text). Devolve
// { biennium, themes: [{theme_title, status, area}] }. Degrada sem chave.
async function extractAgendaThemes(actText) {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return { biennium: null, themes: [], skipped: "no_api_key" };

  const prompt = [
    "Voce e analista regulatorio. O texto abaixo e um ato oficial do DOU que",
    "aprova ou atualiza a AGENDA REGULATORIA de uma agencia reguladora — a lista",
    "de temas/atividades regulatorias que ela planeja tratar no bienio.",
    "Extraia a lista de temas. Responda APENAS com JSON valido no formato:",
    '{"biennium": "2026-2027 ou null", "themes": [{"theme_title": "tema", "status": "planejado|em consulta|concluido ou null", "area": "superintendencia/area ou null"}]}',
    "Se o ato NAO itemizar temas (apenas aprova, sem lista), retorne themes: [].",
    "NAO invente temas — extraia somente o que consta no texto.",
    "",
    `TEXTO: ${String(actText || "").slice(0, 12000)}`
  ].join("\n");
  ...
  try {
    const response = await fetch(API_URL, {
      ...
      body: JSON.stringify({ model: DEFAULT_MODEL, max_tokens: 2000, messages: [{ role: "user", content: prompt }] })
    });
    const data = await response.json();
    if (!response.ok) return { biennium: null, themes: [], error: data?.error?.message };
    const text = data?.content?.[0]?.text || "";
    const match = text.match(/\{[\s\S]*\}/);
    const parsed = match ? JSON.parse(match[0]) : {};
    const themes = Array.isArray(parsed.themes)
      ? parsed.themes.filter((t) => t && typeof t.theme_title === "string" && t.theme_title.trim())
        .map((t) => ({ theme_title: t.theme_title.trim().slice(0, 300), status: t.status || null, area: t.area || null }))
      : [];
    return { biennium: parsed.biennium || null, themes };
  } catch (error) {
    return { biennium: null, themes: [], error: error.message };
  }
}

// M27 — ANGULO de abordagem de patrocinio (Eventos F-EVT4). Recebe { evento, empresa,
// evidencia, ref_notes } e devolve um texto CURTO e factual p/ o time do IRIS usar ao
// convidar a empresa a patrocinar. Usa SO a evidencia publica + os arquivos de referencia
// (won-language/objecoes de negocios passados) — nao inventa. Anti-lobby: nunca sugere
// influenciar decisao publica; e fit de PUBLICO/pegada, nao troca de favor. Degrada sem
// chave: { angulo: null, skipped }. Ver docs/patrocinio-score.md.
async function sponsorAngle(payload) {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return { angulo: null, confidence: 0, skipped: "no_api_key" };

  const serialized = JSON.stringify(payload || {}).slice(0, 9000);
  const prompt = [
    "Voce ajuda um INSTITUTO SEM FINS LUCRATIVOS (IRIS) a redigir um ANGULO curto e",
    "factual p/ convidar uma empresa a PATROCINAR um seminario. Abaixo, em JSON: o",
    "evento, a empresa, a EVIDENCIA de dados publicos (contratos com orgaos do tema,",
    "doacoes, porte, CNAE) e ARQUIVOS DE REFERENCIA (linguagem que ja fechou negocio e",
    "objecoes comuns, vindos de negocios passados do instituto).",
    "Escreva 2-4 frases: por que ESTA empresa tem fit com o publico/tema do evento e",
    "qual gancho de valor usar. Ancore no dado (cite o sinal concreto: 'atua junto a",
    "<orgao>', 'porte <x>'). Use a won-language quando fizer sentido; enderece 1 objecao",
    "se relevante.",
    "REGRAS: (1) NAO invente — use so o que esta no JSON; se a evidencia for escassa,",
    "diga que o fit e fraco. (2) Anti-lobby: e fit de PUBLICO e pegada setorial, NUNCA",
    "sugira influenciar/pressionar orgao publico nem troca de favor regulatorio.",
    "Responda APENAS com JSON valido: {\"angulo\":\"2-4 frases\",\"confidence\":0.0}",
    "",
    `DADOS: ${serialized}`
  ].join("\n");
  ...
  try {
    const response = await fetch(API_URL, {
      method: "POST",
      headers: { "content-type": "application/json", "x-api-key": apiKey, "anthropic-version": "2023-06-01" },
      body: JSON.stringify({ model: DEFAULT_MODEL, max_tokens: 500, messages: [{ role: "user", content: prompt }] })
    });
    const data = await response.json();
    if (!response.ok) return { angulo: null, confidence: 0, error: data?.error?.message };
    const text = data?.content?.[0]?.text || "";
    const match = text.match(/\{[\s\S]*\}/);
    const parsed = match ? JSON.parse(match[0]) : {};
    return { angulo: (parsed.angulo && String(parsed.angulo).trim()) || null, confidence: Number(parsed.confidence) || 0 };
  } catch (error) {
    return { angulo: null, confidence: 0, error: error.message };
  }
}

module.exports = { analyzeAto, summarizeDossier, narrateDeal, extractAgendaThemes, sponsorAngle };
